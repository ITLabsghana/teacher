    ...
    <div className="flex min-h-screen bg-background">
      <aside className="w-64 bg-card border-r border-border p-4 hidden md:flex flex-col">
        ...
        <nav className="flex flex-col gap-2 flex-grow">
          ...
          <NavLink href="/dashboard/reports" icon={<FileText className="h-5 w-5" />} label="Reports & Data" />
          {currentUser.role !== 'Viewer' && (
            <NavLink href="/dashboard/users" icon={<Users className="h-5 w-5" />} label="Manage Users" />
          )}
        </nav>
        <div className="mt-auto">
          <Button variant="outline" className="w-full justify-start text-destructive border-destructive hover:bg-destructive/10 hover:text-destructive" onClick={handleLogout}>
