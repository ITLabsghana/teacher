...
import { deleteTeacher as dbDeleteTeacher, getTeacherById, getSchools, getLeaveRequests } from '@/lib/supabase';
...
export default function TeacherProfile({ initialTeacher, initialSchools, initialLeaveRequests }: TeacherProfileProps) {
    ...
    // This function can be used to refetch data on the client after a mutation
    const refreshData = async () => {
        try {
            const [teacherData, schoolData, leaveData] = await Promise.all([
                getTeacherById(teacherId), // No admin privileges needed on client
                getSchools(),
                getLeaveRequests()
            ]);
            ...
            console.error("Failed to refresh teacher details", error);
            toast({ variant: 'destructive', title: 'Error', description: "Could not refresh teacher profile." });
        }
    };
    ...
    const handleFormSave = () => {
        setIsFormOpen(false);
        refreshData(); // Re-fetch data after save
    }
        ...
        <div className="space-y-6">
            ...
            <Card>
                ...
                <CardContent>
                    ...
                    {Array.isArray(teacher.documents) && teacher.documents.length > 0 && (
                        <>
                            <Separator className="my-4" />
